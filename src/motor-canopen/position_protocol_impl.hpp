...
bool PositionProtocol<id>::update(MotorState &state, MessageCallback &&) {
  ...
  state.status_.setBit<StatusBits::TargetReached>(inPositionWindow_ >=
                                                  positionWindowTime_);
  return true;
}
...
constexpr void PositionProtocol<id>::registerHandlers(
  ...
  map.template setWriteHandler<PositionObjects::PositionWindow>(
      +[](uint32_t value) {
        positionWindow_ = state.scalingFactors_.position.toInternal(value);
        return SdoErrorCode::NoError;
      });

  map.template setWriteHandler<PositionObjects::PositionPID_kP>(
      +[](float value) {
  ...
  map.template setWriteHandler<PositionObjects::PositionPID_kI>(
      +[](float value) {
  ...
  map.template setWriteHandler<PositionObjects::PositionPID_kD>(
      +[](float value) {
  ...
  map.template setWriteHandler<PositionObjects::PositionPID_MaxErrorSum>(
      +[](float value) {
